...
> = {
  // Batches start at the audio step (yt-dlp doesn't apply here), so the
  // "downloading" entry stays unused — kept here for type completeness.
  downloading: AudioLines,
  audio: AudioLines,
  transcript: FileText,
  clean: Sparkles,
  split: Scissors,
...
function childMeta(child: ChildRow) {
  ...
  // Batch concurrency=1: queued children sit at audio/0, active child
  // bumps to 1 the moment ffmpeg's semaphore slot is acquired, then ticks
  // real percent. Three "audio" states matter to the UI.
  const queuedForExtract = statusVal === "audio" && progress === 0;
  const startingExtract = statusVal === "audio" && progress === 1;
  const running = progress > 1 && progress < 100;

  if (queuedForExtract) {
    return {
      label: "Waiting in queue",
      sublabel: "Will start once the current extract finishes",
  ...
  if (startingExtract) {
    return {
      label: "Starting extract…",
      sublabel: "Spinning up ffmpeg",
...
/**
 * Multi-file batch overview — rendered when /new?job=<id> resolves to a
 * parent Job (kind="batch"). Lists children with per-step status and a
 * Continue link into each one's individual wizard.
 *
 * Direct sibling of `PlaylistOverview` — same shape, different starting
 * step (audio vs downloading), different icon, polls /api/jobs/[id]/batch.
 */
export function BatchOverview({ parentId }: { parentId: string }) {
  const [data, setData] = useState<BatchData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [processing, setProcessing] = useState(false);
  const [processError, setProcessError] = useState<string | null>(null);
  // Set of expanded child IDs — multiple can be open at once so the user
  // can compare children side-by-side.
  ...
  async function startProcessAll() {
    ...
    // Leave processing=true once kicked off — the polling loop reflects
    // per-child progress; the button stays disabled to avoid double-sends.
  }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const payload = (await r.json()) as BatchData;
        if (!cancelled) {
          setData(payload);
          setError(null);
        }
    ...
    const id = window.setInterval(() => {
      // Keep polling while ANY child is still mid-pipeline. Once every
      // child is either at render/done or failed, polling stops.
      setData((prev) => {
        if (
          prev &&
    ...
    return (
      <div className="max-w-3xl mx-auto w-full text-center py-16 px-6">
        <p className="text-[15px] text-rose-700 font-medium">
          Couldn&rsquo;t load this batch: {error}
        </p>
        <Link
          href="/"
  ...
  return (
    <div className="max-w-4xl mx-auto w-full px-6 py-10 md:py-12 flex flex-col gap-7 animate-reveal">
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-indigo-50/40 via-white to-fuchsia-50/30 shadow-[0_1px_2px_rgba(9,9,11,0.04)] p-6"
      >
        <div
          aria-hidden
          className="absolute -right-16 -top-16 w-48 h-48 rounded-full bg-gradient-to-br from-indigo-500/10 to-fuchsia-500/10 blur-3xl"
        />
        <div className="relative flex items-start gap-4 flex-wrap">
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            <p className="text-[13.5px] text-slate-500 mt-1 nums leading-tight">
              {total} files{totalDur > 0 ? ` · ${formatDuration(totalDur)} total` : ""} ·{" "}
      ...
      {/* Process-all CTA — appears when at least one child still has
          stages left after audio extraction. Hidden once everything is
          past split (i.e., only render/done/failed remain). */}
      {(() => {
        const pending = data.children.filter((c) => {
          const s = c.status?.status ?? "audio";
          return s !== "render" && s !== "done" && s !== "failed";
        });
          ...
          <div className="rounded-xl border border-slate-200 bg-gradient-to-br from-indigo-50/60 via-white to-fuchsia-50/40 p-5 flex items-center gap-4 flex-wrap">
            <div className="flex-1 min-w-[220px]">
              ...
              <p className="text-[12.5px] text-slate-500 mt-1 leading-snug">
                ...
                {pending.length} remaining file{pending.length === 1 ? "" : "s"}.
      ...
      <p className="text-[12.5px] text-slate-400 text-center">
        Click any file to expand and inspect its audio, transcript, cleanup,
